  ...
  return (
    <div 
      className={cn(
        ...
        !title ? 'bg-[#f3f8ff] dark:bg-[#1a2744] rounded-2xl' : 'bg-white'
      )}>
      { title !== undefined && <h1 className="mb-6 text-xl font-semibold font-poppins text-gray-900 dark:text-white">{title}</h1> }
      <div>
        {children}
      </div>
    </div>
